...
include '../library/configuration.php'; 
        ...
        if (mysqli_num_rows($check_user) > 0) {
            ...
            $insert = mysqli_query($op, "INSERT INTO users (nama, email, telepon, password, unhash, level, status, verifikasi) VALUES ('$post_nama', '$post_email', '$post_telepon', '$hash_password', '$post_password', 'Admin', 'Active', '".random_number(6)."')");
            if ($insert == TRUE) {
                include '../assets/vendor/PHPMailer/classes/class.phpmailer.php';
                whatsapp($post_telepon, 'Akun Admin Berhasil didaftarkan.
...
Akses: '.$cf['url'].'');
                email($post_email, 'Developer '.$cf['name'], 'Pendaftaran Akun Admin '.$cf['name'], 'Akun Admin Berhasil didaftarkan.<br><br>Nama: '.$post_nama.'<br>Email: '.$post_email.'<br>Telepon: '.$post_telepon.'<br>Password: '.$post_password.'<br>Akses: <a href="'.$cf['url'].'">Here</a>');
        		$result = "Akun Admin Berhasil didaftarkan.<br><br>Nama: ".$post_nama."<br>Email: ".$post_email."<br>Telepon: ".$post_telepon."<br>Password: ".$post_password."<br>Akses: ".$cf['url'];
            } else {
          ...
          <div class="card">
            <div class="card-body">
                    ...
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title">Daftarkan Admin</h5>
              ...
              <table class="table datatable">
                ...
                $check_admin = mysqli_query($op, "SELECT * FROM users WHERE level = 'Admin'"); 
                while($data_admin = mysqli_fetch_assoc($check_admin)) {
                ?>
                  <tr>
                    <th scope="row"><? echo $no++; ?></th>
                    <td><? echo $data_admin['nama']; ?></td>
                    <td><? echo $data_admin['email']; ?></td>
                    <td><? echo $data_admin['telepon']; ?></td>
                    <td>
                    <form method="POST">
                    <input name="id" value="<? echo $data_admin['id']; ?>" hidden>
                    <? if ($data_admin['status'] == 'Active') { ?>
