<?php 
 ...
 $capsule->table('tiket')->where('t_assigned',$_SESSION['user']['id'])->update(['clicked_from_technician'=> 'Y']);
?>
<!-- Begin page -->
                                    ...
                                    <tbody>
                                        <?php
                                            ...
                                            if ($start_date && $end_date) {
                                                ...
                                                $sql = "SELECT * FROM `tiket` WHERE STR_TO_DATE(t_created_date, '%d-%m-%Y %h:%i %p') BETWEEN :start_date AND :end_date AND `t_assigned` = :userID";
                                                $query = $db->prepare($sql);
                                                $query->bindParam(':start_date', $start_date_formatted);
                                                $query->bindParam(':end_date', $end_date_formatted);
                                                $query->bindParam(':userID', $id_teknisi);
                                            } else if ($_GET['1'] == '' AND $_GET['2'] == '') {
                                                $sql = "SELECT * FROM `tiket` WHERE `t_assigned` = :userID";
                                                $query = $db->prepare($sql);
                                                $query->bindParam(':userID', $id_teknisi);
                                            } else if ($_GET['1'] == 0 OR $_GET['1'] == 1 OR $_GET['1'] == 2) {
                                                $priority = $_GET['1'];
                                                $sql = "SELECT * FROM `tiket` WHERE t_priority = :priority AND `t_assigned` = :userID";
                                                $query = $db->prepare($sql);
                                    ...
                                    <div class="d-flex mt-2">
                                        <div class="w-100">
                                            ...
                                            <textarea class="form-control form-control-light mb-2" placeholder="Write message"
                                                    id="t_reply_thread" rows="3"></textarea>
                                            <div class="text-end">
...
function cektiketdata(id) {
    ...
    function tiketupdate() {
        ...
        var reply_thread = $("#t_reply_thread").val();
