                                ...
                                <!-- <div class="row mb-2">
                                    <div class="col-sm-4">
                                        <a href="javascript:void(0);" class="btn btn-danger mb-2" data-bs-toggle="modal"
                                            data-bs-target="#modalusr"><i class="mdi mdi-plus-circle me-2"></i>
                                            Add User</a>
                                    </div>
                                </div> -->
                            ...
                            <select id="department" required class="form-control">
                                ...
                                try {
                                    ...
                                    foreach ($departments as $department) {
                                        echo '<option value="' . $department['nama_department'] . '">' . $department['nama_department'] . '</option>';
                                    }
                            ...
                            <select id="unit" required class="form-control">
                                ...
                                try {
                                    ...
                                    foreach ($units as $unit) {
                                        echo '<option value="' . $unit['singkatan_unit'] . '">' . $unit['singkatan_unit'] . '</option>';
                                    }
                                } catch (PDOException $e) {
                                    echo 'ERROR: ' . $e->getMessage();
                                }
...
<!-- END wrapper -->
        ...
        <!-- Modal -->
        <div class="modal fade" id="modalusr" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
            aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form action="/api/user_create.php" id="formusersubmit" method="post">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="staticBackdropLabel">Insert User</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
                    </div> <!-- end modal header -->
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="department" class="mb-1">Username : </label>
                            <input class="form-control" required name="username" type="text">
                        </div>
                        <div class="form-group">
                            <label for="department" class="mb-1">Password : </label>
                            <input class="form-control" required name="password" type="password">
                        </div>
                        <div class="form-group">
                            <label for="department" class="mb-1">Fullname : </label>
                            <input class="form-control" required name="name" type="password">
                        </div>
                        <div class="form-group">
                            <label for="department" class="mb-1">Phone : </label>
                            <input class="form-control" required name="number_phone" type="text">
                        </div>
                        <div class="form-group">
                            <label for="department" class="mb-1">Email : </label>
                            <input class="form-control" required name="email" type="text">
                        </div>
                        <div class="form-group">
                            <label for="department" class="mb-1">Department : </label>
                            <select id="department" required name='department' class="form-control">
                                <?php
                                try {
                                    $sql = "SELECT id_department, nama_department FROM department";
                                    $stmt = $db->prepare($sql);
                        ...
                        <div class="form-group">
                            <label for="department" class="mb-1">Unit : </label>
                        ...
                        <div class="form-group">
                            <label for="department" class="mb-1">Akses : </label>
                            <select id="hkupdate" name="hakakses" required class="form-control">
                                <option value="admin">Admin</option>
                                <option value="teknisi">Teknisi</option>
                                <option value="manajer">Manajer</option>
                                <option value="member">Member</option>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-success" id="usrsubmit">Submit</button>
                    </div> <!-- end modal footer -->
                </div> <!-- end modal content-->
                </form>
            </div> <!-- end modal dialog-->
...
<script type="text/javascript">
    ...
    $("#formusersubmit").submit(function(event){
        ...
        $.ajax({
            ...
            contentType: false,
            beforeSend: function(){
                $("#formusersubmit button[type='submit']").attr('disabled',true);
                $("#formusersubmit button[type='submit']").html('Please wait...');
            },
            success: function(data){
                $("#formusersubmit button[type='submit']").attr('disabled',false);
                $("#formusersubmit button[type='submit']").html('Submit');
                alert(data.message)
                if(data.status){
