...
<div class="row">
        ...
        if (isset($_POST['pesan'])) {
        	...
        	$post_kampus = $op->real_escape_string(trim(stripslashes(strip_tags(htmlspecialchars($_POST['kampus'],ENT_QUOTES)))));
        	...
        	$check_destinasi = mysqli_query($op, "SELECT * FROM destinasi_kampus WHERE id = '$post_destinasi'");
        	...
        	if (mysqli_num_rows($check_pesanan) > 0 OR mysqli_num_rows($check_customer) > 0) {
        	    ...
        	    $insert = mysqli_query($op, "INSERT INTO pesanan (id_user,id_destinasi_kampus,tipe,kampus,catatan,patokan,harga,pembayaran,latitude_customer,longitude_customer) VALUES ('".$data_user['id']."','$post_destinasi','Berangkat','$post_kampus','$post_catatan','$post_patokan','".$data_destinasi['harga'.$post_kampus.'']."','$post_pembayaran','$post_latitude','$post_longitude')");
            	if ($insert == TRUE) {
        ...
        ?>
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Berangkat Kampus</h5>
              <form method="POST">
                ...
                <label for="kampus" class="col-sm-12 col-form-label">Tujuan </label>
                <div class="selectgroup">
                    <label class="selectgroup-item">
                        <input type="radio" name="kampus" value="1" class="selectgroup-input" required>
                        <span class="selectgroup-button">Kampus 1</span>
                    </label>
                    <label class="selectgroup-item">
                        <input type="radio" name="kampus" value="2" class="selectgroup-input" required>
                        <span class="selectgroup-button">Kampus 2</span>
                    </label>
                </div>
                <div class="row mb-3">
                    <label for="destinasi" class="col-sm-12 col-form-label">Lokasi Berangkat</label>
                    <select id="destinasi" name="destinasi" class="form-control select2 form-select" required>
                        <option value="">Pilih kampus tujuan...</option>
                    </select>
                </div>
                <small>Tidak menemukan lokasi anda? <a href="<? echo $cf['url']; ?>customer/destinasi_kampus_input">Tambahkan sekarang!</a></small>
                <div id="note"></div>
...
$js .= '
  ...
  $("#destinasi").change(function() {
        var destinasi = $(this).val();
        var kampus = $("input[name=\"kampus\"]:checked").val();
        $.ajax({
            url: "berangkat_note.php",
            type: "POST",
            data: {
                destinasi: destinasi,
                kampus: kampus 
    ...
    $("input[name=\"kampus\"]").change(function() {
        var kampus = $(this).val();

        $.ajax({
            url: "berangkat_kampus.php",
            type: "POST",
            data: { kampus: kampus },
            dataType: "html",
            success: function(msg) {
                $("#kampus_pilihan").html(msg);
            }
        });
