...
<div class="row">
        ...
        if (isset($_POST['pesan'])) {
        	...
        	$post_jenis = $op->real_escape_string(trim(stripslashes(strip_tags(htmlspecialchars($_POST['jenis'],ENT_QUOTES)))));
        	$post_penerima_nama = $op->real_escape_string(trim(stripslashes(strip_tags(htmlspecialchars($_POST['penerima_nama'],ENT_QUOTES)))));
        	$post_penerima_kontak = $op->real_escape_string(trim(stripslashes(strip_tags(htmlspecialchars($_POST['penerima_kontak'],ENT_QUOTES)))));
        	...
        	if (mysqli_num_rows($check_pesanan) > 0 OR mysqli_num_rows($check_customer) > 0) {
        	    ...
        	    $insert = mysqli_query($op, "INSERT INTO pesanan (id_user,id_destinasi_lain,id_jenis,pemberangkatan,tipe,penerima_nama,penerima_kontak,patokan,harga,pembayaran,latitude_customer,longitude_customer) VALUES ('".$data_user['id']."','$post_destinasi','$post_jenis','$post_pemberangkatan','Paket','$post_penerima_nama','$post_penerima_kontak','$post_patokan','".$data_destinasi['harga']."','$post_pembayaran','$post_latitude','$post_longitude')");
            	if ($insert == TRUE) {
        ...
        ?>
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Pengantaran Paket</h5>
              <form method="POST">
                ...
                <div id="note"></div>
                <label for="jenis" class="col-sm-12 col-form-label">Jenis Paket</label>
                <select id="jenis" name="jenis" class="form-control form-select" required>
                    <option value="">Pilih salah satu...</option>
                    ...
                    <option value="<? echo $data_kategori['id']; ?>"><? echo $data_kategori['nama']; ?></option>
                    <? } ?>
                </select>
                ...
                <label for="penerima_nama" class="col-sm-12 col-form-label">Nama Penerima </label>
                <input type="text" name="penerima_nama" class="form-control" required>
                <label for="penerima_kontak" class="col-sm-12 col-form-label">Nomor Telepon Penerima </label>
                <input type="number" name="penerima_kontak" class="form-control" required>
                <label for="patokan" class="col-sm-12 col-form-label">Alamat Lengkap Penerima </label>
